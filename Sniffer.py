...
class PacketSniffer:
    ...
    def __init__(self, config: dict):
        ...
        self.flow_timeout: float = float(config["capture"]["flow_timeout_seconds"])
        self.max_active_flows: int = int(config["capture"]["max_active_flows"])

        # Active flows currently being assembled, keyed by FlowKey.
        self._active_flows: dict[FlowKey, Flow] = {}

        # Thread-safe queue of flows that are finished and ready to be
        # consumed by stream_flows(). We use a simple list + lock rather
        # than queue.Queue so we can also do periodic timeout sweeps.
    ...
    def _process_one_packet(self, timestamp: float, packet: Packet) -> None:
        """
        The actual flow-assembly logic for a single packet. This is
        identical in behaviour to the original synchronous
        _handle_packet, just renamed and run from the worker thread
        instead of directly from Scapy's capture callback, and using
        the timestamp captured at enqueue time (closer to the true
        arrival time than whenever the worker happens to process it).
        """
        if IP not in packet:
    ...
    # ------------------------------------------------------------------
    # Internal: flow lifecycle management
    # ------------------------------------------------------------------
