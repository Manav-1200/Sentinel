...
| 2 | Intelligence | Supervised ML + LLM log analysis + self-labelling pipeline + port-scan detection | Project 1 v2 | ✅ Complete — 84 passing tests (verified end-to-end on real nmap scans, real floods, real DDoS traffic) — tagged `v2.0-supervised-learning` |
...
- [ ] Achieve at least 70% test coverage across all modules — as of Phase 2's close, `PortScanTracker`, `Labeller`'s port-scan/DDoS methods, `AttackClassifier`, and `LLMAnalyser` are verified manually/functionally but do not yet have dedicated automated test coverage; this is real, tracked follow-up work for this phase, not assumed already done
...
- [ ] **Dedicated automated test coverage for Phase 2's newer modules:** `PortScanTracker`, `Labeller.process_port_scan_attack()`, and the hard-timeout backstop in `LLMAnalyser` are all verified manually/functionally (real nmap scans, a real hung-LLM reproduction and fix) but don't yet have dedicated `pytest` coverage the way Phase 1's modules do. Tracked here explicitly rather than left implicit, so it isn't silently assumed done — see Phase 5.5.
