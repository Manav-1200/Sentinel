...
- [ ] **Classifier is currently effectively untrained** — after clearing 625 stale-schema rows, only ~82 LLM-labelled current-schema samples remain with heavy class imbalance. Needs more diverse current-schema labelled data accumulated before retraining is meaningful. Still the highest-priority open item — a code fix can't compensate for too little training data.
- [x] **Classifier mislabelling large legitimate bulk transfers as "ddos" — fixed in code, still needs a live re-check.** Turned out to be two stacked bugs. First: missing features — added `fwd_packet_share` and `ack_ratio` to `features/extractor.py` so a backward-heavy, high-ACK legitimate download can actually be told apart from a forward-heavy, low-ACK flood, since raw packet count/rate alone never could. Second, sneakier one found right after: `_get_feature_order()` was picking its column order from whichever labelled sample happened to be first in the dataset — with 706 old samples and only a handful collected after the new features existed, it kept quietly training without them. No error, no warning. That's exactly why a real 68,245-packet legitimate transfer was still coming back "ddos" even after the first fix supposedly landed. Fixed by having `train()` figure out the most common feature schema across all usable samples, drop anything that doesn't match, and print a loud warning saying how many got excluded and why — same "don't trust a clean run, verify the mechanism" lesson as the port_scan_tracker/TRAINING_LABEL_SOURCES stuff below. Still need to actually re-run the original bulk-transfer repro against real traffic to confirm this holds, not just trust the code review.
